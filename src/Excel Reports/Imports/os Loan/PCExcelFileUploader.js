...
const PCExcelFileUploader = ({ onDataUpload, prCooperative }) => {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const loanValueIndex = firstRow.findIndex(
        (cell) => cell?.toLowerCase() === "OS Loan".toLowerCase()
      );
      ...
      const mappedData = jsonData
        ...
        .map((row) => ({
          osLoanValue:
            typeof row[loanValueIndex] === "number"
              ? row[loanValueIndex] < 0
                ? row[loanValueIndex] * -1
                : row[loanValueIndex]
              : 0,
