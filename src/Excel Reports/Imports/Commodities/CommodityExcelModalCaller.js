...
const CommodityExcelModalCaller = ({ prCooperative }) => {
  ...
  const handleSubmit = async () => {
    ...
    async function fetchData(item) {
      try {
        await API.post(`/commodity/add`, item).then(
    ...
    <>
      <Button onClick={() => setModalOpen(true)}>
        ...
        <span className="whitespace-nowrap">
          ...
          <span className="whitespace-nowrap">Commodity Import</span>
        </span>
      </Button>
      <Modal open={modalOpen} size="tiny" onClose={handleModalClose}>
        <Modal.Header>Commodity Import</Modal.Header>
        <Modal.Content>
...
export default CommodityExcelModalCaller;
