...
const PaipUpExcelModalCaller = ({ prCooperative }) => {
  ...
  const handleSubmit = async () => {
    ...
    async function fetchData(item) {
      try {
        await API.post(`/paidUpShare/add`, item).then(
    ...
    <>
      <Button onClick={() => setModalOpen(true)}>
        ...
        <span className="whitespace-nowrap">
          ...
          <span className="whitespace-nowrap">PaidUp Import</span>
        </span>
      </Button>
      <Modal open={modalOpen} size="tiny" onClose={handleModalClose}>
        <Modal.Header>Paidup Share Import</Modal.Header>
        <Modal.Content>
...
export default PaipUpExcelModalCaller;
