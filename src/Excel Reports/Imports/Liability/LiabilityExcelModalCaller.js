...
const LiabilityExcelModalCaller = ({ prCooperative }) => {
  ...
  const handleSubmit = async () => {
    ...
    async function fetchData(item) {
      try {
        await API.post(`/liability/add`, item).then(
    ...
    <>
      <Button onClick={() => setModalOpen(true)}>
        ...
        <span className="whitespace-nowrap">
          ...
          <span className="whitespace-nowrap">Liability Import</span>
        </span>
      </Button>
      <Modal open={modalOpen} size="tiny" onClose={handleModalClose}>
        <Modal.Header>Liability Import</Modal.Header>
        <Modal.Content>
...
export default LiabilityExcelModalCaller;
