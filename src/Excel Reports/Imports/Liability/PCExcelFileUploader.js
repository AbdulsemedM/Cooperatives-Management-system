...
const PCExcelFileUploader = ({ onDataUpload, prCooperative }) => {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const liabilityValueIndex = firstRow.findIndex(
        (cell) => cell?.toLowerCase() === "Total Liability".toLowerCase()
      );
      ...
      const mappedData = jsonData
        ...
        .map((row) => ({
          liabilityValue:
            typeof row[liabilityValueIndex] === "number"
              ? row[liabilityValueIndex]
              : 0,
