  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const AssetNameIndex = firstRow.findIndex(
        (cell) => cell?.toLowerCase() === "Asset Name".toLowerCase()
      );
      ...
      const mappedData = jsonData
        ...
        .map((row) => ({
          value:
            typeof row[AssetValueIndex] === "number" ? row[AssetValueIndex] : 0,
          assetName: row[AssetNameIndex] ? row[AssetNameIndex] : null,
