...
const AssetExcelModalCaller = ({ prCooperative }) => {
  ...
  const handleSubmit = async () => {
    ...
    async function fetchData(item) {
      try {
        await API.post(`/asset/add`, item).then(
    ...
    <>
      <Button onClick={() => setModalOpen(true)}>
        ...
        <span className="whitespace-nowrap">
          ...
          <span className="whitespace-nowrap">Asset Import</span>
        </span>
      </Button>
      <Modal open={modalOpen} size="tiny" onClose={handleModalClose}>
        <Modal.Header>Asset Import</Modal.Header>
        <Modal.Content>
...
export default AssetExcelModalCaller;
