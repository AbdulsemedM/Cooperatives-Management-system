...
const PCExcelFileUploader = ({ onDataUpload, prCooperative }) => {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const totalCapitalValueIndex = firstRow.findIndex(
        (cell) => cell?.toLowerCase() === "Total Capital".toLowerCase()
      );
      ...
      const mappedData = jsonData
        ...
        .map((row) => ({
          totalCapitalValue:
            typeof row[totalCapitalValueIndex] === "number"
              ? row[totalCapitalValueIndex]
              : 0,
