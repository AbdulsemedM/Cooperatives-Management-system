...
const TotalCapitalExcelModalCaller = ({ prCooperative }) => {
  ...
  const handleSubmit = async () => {
    ...
    async function fetchData(item) {
      try {
        await API.post(`/totalCapital/add`, item).then(
    ...
    <>
      <Button onClick={() => setModalOpen(true)}>
        ...
        <span className="whitespace-nowrap">
          ...
          <span className="whitespace-nowrap">Total Capital Import</span>
        </span>
      </Button>
      <Modal open={modalOpen} size="tiny" onClose={handleModalClose}>
        <Modal.Header>Total Capital Import</Modal.Header>
        <Modal.Content>
...
export default TotalCapitalExcelModalCaller;
