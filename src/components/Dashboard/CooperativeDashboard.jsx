...
const CooperativeDashboard = ({ prCooperativeID }) => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [prCooperativeID, dispatch]);

  const capitalData = useSelector((state) => state.capital);
  const { paidUpShare, osLoan, annualTurnover } = capitalData;
...
export default connect(mapStateToProps)(CooperativeDashboard);
